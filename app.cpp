...
App::App(AppState& state, std::function<void()> on_quit)
    : state(state), on_quit(on_quit) {
    auto top_menu = Menu(&top_menu_entries, &top_menu_selected,
    ...
    auto main_text_input = CatchEvent(main_text_input_base, [this](
                                                                Event event) {
        bool is_start_of_line =
            (event == Event::Home || event == Event::ArrowLeftCtrl ||
             event == Event::CtrlA || event == Event::Special("\x1b[1;9D") ||
             event == Event::Special("\x1b[1;3D"));
        bool is_end_of_line =
            (event == Event::End || event == Event::ArrowRightCtrl ||
             event == Event::CtrlE || event == Event::Special("\x1b[1;9C") ||
             event == Event::Special("\x1b[1;3C"));
    ...
    file_option.on_enter = [this] {
        if (file_selected == FILE_SAVE) {
            show_save_modal = true;
        } else if (file_selected == FILE_QUIT) {
            this->on_quit();
        }
    };
    ...
    edit_option.on_enter = [this] {
        if (edit_selected == EDIT_COPY) {
            clipboard = this->state.main_text_content;
        } else if (edit_selected == EDIT_PASTE) {
            this->state.main_text_content += clipboard;
        }
    };
    ...
    auto tab_container =
        Container::Tab({file_menu, edit_menu, help_menu}, &top_menu_selected);

    auto main_container =
        Container::Vertical({top_menu, tab_container, main_text_input});

    auto main_renderer = Renderer(
        main_container, [this, top_menu, tab_container, main_text_input] {
            return vbox({
                       top_menu->Render(),
                       separator(),
    ...
    auto version_close_button =
        Button("Close", [this] { show_version_modal = false; });
    auto version_modal =
        Renderer(version_close_button, [this, version_close_button] {
            return vbox({
                       text("Version 1.0") | center,
                       separator(),
                       version_close_button->Render() | center,
                   }) |
                   size(WIDTH, GREATER_THAN, 30) | border | clear_under |
                   center;
        });
    ...
    auto save_cancel_button =
        Button("Cancel", [this] { show_save_modal = false; });
    auto save_buttons_container =
        Container::Horizontal({save_button, save_cancel_button});
    auto save_modal_container =
        Container::Vertical({filename_input, save_buttons_container});

    auto save_modal =
        Renderer(save_modal_container,
                 [this, filename_input, save_button, save_cancel_button] {
                     return vbox({
...
ftxui::Component App::GetComponent() { return component; }
